...
static ALLEGRO_BITMAP* winpic = NULL;
static int winpicW;
static int winpicH;
...
static void init()
{
	...
	winpic = load_bitmap("Assets/winscene.png");
	winpicW = al_get_bitmap_width(winpic);
	winpicH = al_get_bitmap_height(winpic);

}

static void draw()
{
	al_draw_text(menuFont, al_map_rgb(255, 255, 255), SCREEN_W / 2, SCREEN_H - 675, ALLEGRO_ALIGN_CENTER, "VICTORY!");
	...
	const float offset_w = (SCREEN_W >> 1) - 0.5 * scale * winpicW;
	const float offset_h = (SCREEN_H >> 1) - 0.75 * scale * winpicH;

	al_draw_scaled_bitmap(winpic, 0, 0, winpicW, winpicH, offset_w, offset_h, winpicW * scale, winpicH * scale, 0);
