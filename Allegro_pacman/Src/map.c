...
static void draw_kill_bean(Map* M, const int row, const int col);
...
/* ----------------------------- start of default map code*/
	...
	if (filepath == NULL) {
		...
		{
		case 1:
			pFile = fopen("Assets/map_one_piece.txt", "r");
			if (!pFile) {
				game_abort("error to open map file\n");
				return NULL;
			}
...
static void draw_block_index(Map* M, const int row, const int col) {
	...
	dw = block_width / 3;
		...
		al_draw_filled_rectangle(s_x, s_y,
			e_x, e_y, al_map_rgb(227, 218, 201));
		...
		if (col > 0 && L && !(UL && BL && U && B)) {
			...
			e_y = s_y + dw;
			al_draw_filled_rectangle(s_x, s_y,
				e_x, e_y, al_map_rgb(227, 218, 2501));
		}
	}
}

static void draw_bean(Map* M, const int row, const int col) {
	al_draw_filled_circle(map_offset_x + col * block_width + block_width / 2.0, map_offset_y + row * block_height + block_height / 2.0, block_width/6.0,  al_map_rgb(255, 255, 255));
}

static void draw_power_bean(Map* M, const int row, const int col) {
	al_draw_filled_circle(map_offset_x + col * block_width + block_width / 2.0, map_offset_y + row * block_height + block_height / 2.0, block_width / 3.0, al_map_rgb(255, 255, 51));
}

static void draw_speed_bean(Map* M, const int row, const int col) {
	al_draw_filled_circle(map_offset_x + col * block_width + block_width / 2.0, map_offset_y + row * block_height + block_height / 2.0, block_width / 3.0, al_map_rgb(50, 242, 255));
}
static void draw_kill_bean(Map* M, const int row, const int col) {
	al_draw_filled_circle(map_offset_x + col * block_width + block_width / 2.0, map_offset_y + row * block_height + block_height / 2.0, block_width / 3.0, al_map_rgb(255, 0, 0));
}
