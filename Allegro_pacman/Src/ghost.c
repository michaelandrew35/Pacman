...
Ghost* ghost_create(int flag) {
	...
	//if (ghost->dead_sprite) game_log("load ghost dead sprite");
	...
	*/
	...
	if (ghost->status == FLEE) {
		...
		if (al_get_timer_count(power_up_timer) <= 7) {
			...
			//printf("%d", ghost->objData.moveCD % 32);
			if (ghost->objData.moveCD % 32 >= 0 && ghost->objData.moveCD % 32 < 8)
				al_draw_scaled_bitmap(ghost->flee_sprite, 0, 0, 16, 16, drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y, draw_region, draw_region, 0);
			else if (ghost->objData.moveCD % 32 >= 8 && ghost->objData.moveCD % 32 < 16)
			...
			else if (ghost->objData.moveCD % 32 >= 16 && ghost->objData.moveCD % 32 < 24)
				al_draw_scaled_bitmap(ghost->flee_sprite, 16, 0, 16, 16, drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y, draw_region, draw_region, 0);
			else if (ghost->objData.moveCD % 32 >= 24 && ghost->objData.moveCD % 32 < 32)
...
void ghost_move_script_GO_OUT(Ghost* ghost, Map* M) {
	...
	if (mapsoption == 2)
	{
		if (M->map[ghost->objData.Coord.y][ghost->objData.Coord.x] == 'B')
			ghost_NextMove(ghost, UP); //TODO CHANGE THIS 
		else
			ghost->status = FREEDOM;
	}
	else
	{
		if (M->map[ghost->objData.Coord.y][ghost->objData.Coord.x] == 'B')
			ghost_NextMove(ghost, RIGHT); //TODO CHANGE THIS 
		else
			ghost->status = FREEDOM;
	}
}
